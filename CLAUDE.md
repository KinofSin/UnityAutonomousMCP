...
- **VRChat's stats count *disabled* renderers.** A wardrobe toggle that is switched off still costs rank and download size (LEAF: ~110,800 of 231,895 verts sit on disabled objects). Deleting an unused toggle beats any importer flag — but it removes functionality, so it always asks.
