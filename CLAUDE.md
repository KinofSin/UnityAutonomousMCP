...
- **`manage_project_template`** (`inspect`/`list`/`apply`): sets a VRChat avatar project to a pro baseline — `inspect` reports each scene avatar's state (PC/Quest + per-step done flags), `apply` idempotently adds only what's missing (VRC descriptor + viewpoint, Expression Menu/Parameters, `Assets/_Project/<Avatar>/` folders). Non-destructive; VRCSDK components added via reflection (`Editor/Templates/VrcReflection.cs`), skipped with a note if the SDK is absent. Pure diff engine in `Editor/Templates/ProjectTemplateEngine.cs`.
