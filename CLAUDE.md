...
- **Dev loop — recompiling package edits in Leaf (SOLVED 2026-05-31 via junction embed).** The package is mounted into Leaf as an **embedded** package via a Windows directory **junction** `Leaf\Packages\com.autonomous.unity.mcp` → the repo's `com.autonomous-unity.mcp` folder (the old `file:` manifest entry was removed). Workflow: **edit in the repo → press `Ctrl+R` in a focused Unity → it recompiles from live source** (errors surface against the `Packages\com.autonomous.unity.mcp\…` path). Optionally enable Preferences → Asset Pipeline → **Auto Refresh** so a focus alone recompiles.
  ...
  - **Still true:** Unity **defers all compilation while unfocused**, so driving `refresh_unity` purely over the bridge (editor in background) will NOT compile — the user must focus Unity (`Ctrl+R`) to apply edits, *then* verify over the bridge. `get_compilation_errors`/tests read the stale last-good assembly until that focused recompile happens. Prove an edit is live before trusting verification (deliberate-error probe, or a future build-stamp in `health_check`).
