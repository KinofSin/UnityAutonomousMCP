...
- **Avatar Cleanup window** (`Window/Autonomous MCP/Avatar Cleanup`, or the Advisor's **Cleanup** button) — the same `AvatarCost` model the `cost` section reads, rendered as a sortable table with tick-boxes and a live "leaves N polys (rank)" footer. **This is the one place the MCP UI edits the scene itself rather than queuing consent**, and only because deleting a scene object is natively undoable: every removal is a checkpoint *plus* one collapsed `Undo` step. Asset edits stay with the agent — they are not undoable. It refuses to delete the avatar root, warns loudly when a selected object is currently **enabled**, and warns that deleting a child of a prefab instance is an override that a prefab revert undoes. If the checkpoint throws it deletes nothing rather than relying on Undo alone.
- **`dossier` section `cost` — per-object cost attribution.** Answers "what should I delete?" without hand-writing a query: every renderer-holding object with its polygons, material slots, share of total, and `ifRemoved` (the totals *and rank* left behind without it), sorted by cost, plus an `inactive` roll-up with `ifAllRemoved`. **Rank thresholds live only in `StateDossier.PolygonTier`/`MaterialSlotTier`/…; `PcBudgets` is projected from them.** `"Over"` means past the highest tier VRChat publishes for that metric (polygons have no `medium`, so nothing invents one).
- **VRChat's stats count *disabled* renderers**, so a switched-off wardrobe toggle still costs rank and download size. **But quantify in polygons, not verts — they rank differently.** On LEAF the disabled objects are 86,619 of 278,879 *polys* (31%), while by *verts* they look like ~48%: `ANIMAL DOG` is the biggest mesh by verts (43,000) yet only 8th by polygons (20,315). Ranking removal candidates by vertex count picks the wrong target. And check `ifRemoved` before promising a win: deleting **every** disabled object on LEAF leaves 192,260 polys, still far past the 70,000 `good` ceiling, so it changes download size and memory but **not** the rank.
