...
- `.claude/tools/unity-verify.mjs` — `refresh_unity` + `read_console {level:"error"}` (never `get_compilation_errors`). Exit `0` clean / `1` errors / `2` bridge down. Runs unattended: the editor recompiles in the background, no focus needed.
