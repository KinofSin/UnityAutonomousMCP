...
- **`active` in the `cost` section is measured *within* the avatar, not `activeInHierarchy`.** VRChat scenes park every avatar root disabled and enable one at a time, so `activeInHierarchy` is false for every renderer on every avatar — read removal candidates off that and a parked avatar offers its **entire body** as free space. Caught live: the cleanup window showed `ALLEXSI` as "19 undriven disabled objects (361,630 polys)" with a **Select undriven (19)** button that would have deleted the whole avatar. `rootActive:false` says nothing below is visible; enable the root before judging anything visually. Raw `activeInHierarchy` is still reported alongside.
